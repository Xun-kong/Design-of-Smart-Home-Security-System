...
u8 humi,temp,m=40,n=70,t,h,x=0,dat,read;
/*ASCII字模数据*********************/
/*宽8像素，高16像素*/
...
void OLED_ShowNum(u8 X,u8 Page,u8 Num,u8 FontSize)
{
	char num[8];
	sprintf(num,"%d",Num);
	OLED_ShowString(X,Page,num,FontSize);
}
...
void OLED_Page1(void)
{
	OLED_Init();
	OLED_Clear();
  OLED_ShowChinese(0, 0, "智能家庭安防系统");
	OLED_ShowString(0,3,"v1.0 2026/02/09",8);
	OLED_ShowChinese(5,6,"赣￥东￥学￥院");
	delay_s(2);
	OLED_Clear();
}
...
void OLED_Page2(void)
{
	...
	OLED_ShowChar(32,0,':',8);
	OLED_ShowImage(56,0,14,2,Image);	//单位符号 "℃"
	OLED_ShowChinese(0,18,"湿度");
	OLED_ShowChar(32,18,':',8);
	OLED_ShowString(56,18,"%RH",8);		//单位符号  "%RH"
	
	OLED_ShowNum(40,0,temp,8);				//显示温度
	OLED_ShowNum(40,18,humi,8);				//显示湿度
}

void OLED_Page3(void)
{
	OLED_ShowChinese(0,6,"温度阈值");
	OLED_ShowChar(64,6,':',8);
	OLED_ShowImage(88,6,14,2,Image); //单位符号 "℃"
}
void OLED_Page4(void)
{
	OLED_ShowChinese(0,6,"湿度阈值");
	OLED_ShowChar(64,6,':',8);
	OLED_ShowString(88,6,"%RH",8);
}

void OLED(void){
	
	
	if (read==1)	DHT11_Read_Data(&humi,&temp),read=0;	//读取温湿度数据	
	if (dat==1)	OLED_Page2(),dat=0;							//显示温湿度
	
/***********************按键控制温湿度报警阈值*********************/
	if (x==1) OLED_Page3();//控制OLED显示  “温度阈值：”
	if (x==2) OLED_Page4();//控制OLED显示  “湿度阈值：”
	if (x==3) x=0,OLED_ShowString(0,6,"                ",8);
	if (x>=4) x=0;
	if (x==1) OLED_ShowNum(72,6,m,8);//显示温度阈值数值，默认值为m
  if (x==2) OLED_ShowNum(72,6,n,8);//显示湿度阈值数值，默认值为n
	if (x==0) t=m,h=n;
}
